...
public class AuthRestAPIs {
	...
	public ResponseEntity<String> registerUser(@Valid @RequestBody SignUpForm signUpRequest) {
		...
		if (numOfSpecial < 1 || numOfLetters < 1 || numOfDigits < 1) {
			...
			return new ResponseEntity<String>("Fail -> Need at least a number, a letter and a special character ",
					HttpStatus.BAD_REQUEST);
		}

		// Creating user's account
		User user = new User(signUpRequest.getCompany(), signUpRequest.getFirstname(), signUpRequest.getLastname(),
				signUpRequest.getUsername(), signUpRequest.getEmail(), signUpRequest.getQuestion1(),
				signUpRequest.getAnswer1(), signUpRequest.getQuestion2(), signUpRequest.getAnswer2(),
	...
	public ResponseEntity<String> TransferFunds(@Valid @RequestBody TransferRequest transferRequest) throws Exception {
		...
		UserAccount fromaccount = accountRepository.findByAccountno(transferRequest.getSenderaccountno())
				.orElseThrow(() -> new Exception("Sender Account Does Not Exist!"));

		UserAccount toaccount = accountRepository.findByAccountno(transferRequest.getReceiveraccountno())
				.orElseThrow(() -> new Exception("Receiver Account Does Not Exist!"));
		...
		// transRepository.save(new
		// Transactions(Transactions.TransType.FROMACCOUNT,Transactions.TransType.FROMACCOUNT
		// + fromaccount.getAccountno(),
		// transferRequest.getAmount(),0.00,toaccount.getAmount(),
		// toaccount.getUser()));

		// transRepository.save(new
		// Transactions(Transactions.TransType.TOACCOUNT,0.00,transferRequest.getAmount(),fromaccount.getAmount(),
		// fromaccount.getUser()));
	...
	public ResponseEntity<String> CreditCardPayment(@Valid @RequestBody CreditCardPaymentRequest creditRequest)
			throws Exception {
		...
		UserCreditCard creditcard = creditcardRepository.findByCreditcardno(creditRequest.getCreditcardno())
				.orElseThrow(() -> new Exception("Credit Card Does Not Exist!"));
		...
		// transRepository.save(new Transactions(Transactions.TransType.FROMACCOUNT,
		// creditRequest.getAmount(),0.00,creditcard.getAmountavailable(),
		// creditcard.getUser()));

		// transRepository.save(new
		// Transactions(Transactions.TransType.CREDITCARD,0.00,creditRequest.getAmount(),fromaccount.getAmount(),
		// fromaccount.getUser()));
		...
		return ResponseEntity.ok().body("$" + creditRequest.getAmount() + " was successfully paid to CC no. " + "  "
